function request(options, callback) {
    ...
    xhr.addEventListener('load', () => {
        if (xhr.status < 400) { callback(null, xhr.responseText) } else { callback(new Error('Request failed: ' + xhr.status + ' ' + xhr.statusText)) }
    })
...
function send() {
    var body = {
        sender: messageForm.elements.sender.value,
